...
<dict>
    ...
    <array>
        ...
        <!-- Precise location — merchant discovery (foreground only) -->
        <dict>
            <key>NSPrivacyCollectedDataType</key>
        ...
        <!-- Date of birth — profile completion -->
        <dict>
            <key>NSPrivacyCollectedDataType</key>
            <string>NSPrivacyCollectedDataTypeDateOfBirth</string>
            <key>NSPrivacyCollectedDataTypeLinked</key>
            <true/>
            <key>NSPrivacyCollectedDataTypeTracking</key>
            <false/>
...
function withPrivacyManifest(config) {
  return withDangerousMod(config, [
    ...
    async (cfg) => {
      ...
      const appName = cfg.modRequest.projectName || cfg.name || 'jitplus';
      const targetDir = path.join(iosDir, appName);

      // Ensure directory exists (should already from Expo prebuild)
      fs.mkdirSync(targetDir, { recursive: true });

      const manifestPath = path.join(targetDir, 'PrivacyInfo.xcprivacy');
      fs.writeFileSync(manifestPath, PRIVACY_MANIFEST, 'utf-8');
      // eslint-disable-next-line no-console
      console.log(`✅ PrivacyInfo.xcprivacy written to ${manifestPath}`);
