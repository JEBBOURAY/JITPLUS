...
/** Check if a password meets the minimum requirements (8+ chars, uppercase, digit, special). */
...
export function getPasswordStrength(
  ...
  if (score <= 2)
    return { level: 'weak', label: t('setPassword.strengthWeak'), color: '#EF4444', pct: 0.33 };
  if (score <= 3)
    return { level: 'medium', label: t('setPassword.strengthMedium'), color: '#F59E0B', pct: 0.66 };
